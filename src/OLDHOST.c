...
// character from the message. It takes in three parameters: a pointer to a
// struct
//  Man_port_at_host, an array of characters called msg, and a pointer to a
// character called c. The function first reads the command from the manager
// port using the read() function and stores it in the msg array. It then loops
// through the message until it finds a non-space character, which it stores in
// c. It then continues looping until it finds another non-space character,
// which is used to start copying the rest of the message into msg starting at
// index 0. Finally, it adds a null terminator at the end of msg and returns n.
// */
...
//            MAX_MSG_LENGTH); /* Get command from manager */
...
//           struct Response *r = createResponse(resId, PKT_PING_REQ,
//           TIMETOLIVE); addToResList(responseList, r); char
//           resIdStr[PACKET_PAYLOAD_MAX]; snprintf(resIdStr,
//           PACKET_PAYLOAD_MAX, "rid:%d", resId); int resIdStrLen =
//           strnlen(resIdStr, PACKET_PAYLOAD_MAX);
...
//         default:;
//       }
//       // Release semaphore console_print_access once commands have been
//       executed sem_signal(&console_print_access);
//     }
...
//           char filePath[MAX_FILENAME_LENGTH] = {0};
//           int filePathLen =
//               snprintf(filePath, MAX_FILENAME_LENGTH, "./%s/%s",
//               hostDirectory,
...
//             int fileLen = fread(string, sizeof(char), PACKET_PAYLOAD_MAX,
//             fp); fclose(fp); string[fileLen] = '\0'; for (int i = 0; i <
//             fileLen; i++) {
...
//                 n = file_buf_remove(&f_buf_upload, string,
//                 PACKET_PAYLOAD_MAX); string[n] = '\0'; n = fwrite(string,
//                 sizeof(char), n, fp);
//               }
//               fclose(fp);
//             }
