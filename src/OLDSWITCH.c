...
// and an integer ID. It iterates through the maximum number of routes and
// checks if the ID matches the current entry's ID and if it is valid. If it
// finds a match, it returns the index of that entry in the routing table.
// Otherwise, it returns -1 indicating that no valid entry was found.
// */
...
// pointers and the size of the array as parameters. It then iterates through
// the port_array and sends the job's packet to each port in the array, except
// for the port that corresponds to the job's in_port_index.
// */
...
//       (struct TableEntry *)malloc(sizeof(struct TableEntry) *
//       MAX_NUM_ROUTES);
//   for (int i = 0; i < MAX_NUM_ROUTES; i++) {
