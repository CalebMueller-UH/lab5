...
//       MAX_RESPONSE_LEN));
//   snprintf(fullPath, MAX_FILENAME_LENGTH + MAX_RESPONSE_LEN, "%s/%s",
//   directory,
//            fname);
...
//     int pktLen = snprintf(pkt->payload, PACKET_PAYLOAD_MAX, "%s:%.*s",
//     ticket,
//                           bytesRead, buffer);
//     pkt->length = pktLen;
...
//   char *msg = (char *)malloc(sizeof(char) * PACKET_PAYLOAD_MAX - TICKETLEN -
//   1); parsePacket(recPkt->payload, ticket, msg);
...
//           char *fullPath =
//               (char *)malloc(sizeof(char) * 2 * MAX_FILENAME_LENGTH);
//           int fullPathLen = snprintf(fullPath, 2 * MAX_FILENAME_LENGTH,
//           "%s/%s",
//                                      hostDirectory, r->filename);

//           FILE *fp;
//           fp = fopen(fullPath, "w");
//           if (fp == NULL) {
//             fprintf(stderr,
//                     "host%d: handleIncomingResponsePacket: "
//                     "PKT_DOWNLOAD_RESPONSE: unable to create file\n",
//                     host_id);
//           } else {
//             r->reqFp = fp;
//           }
...
//     fprintf(stderr,
//             "ERROR: host%d jobSendRequestHandler invoked with invalid
//             function " "parameter: %s%s%s\n", host_id, !job_from_queue ?
//             "job_from_queue " : "", !host_q ? "host_q " : "", !requestList ?
//             "requestList" : "");
//     return;
//   }
...
//   if (!requestX) {
//     fprintf(stderr,
//             "ERROR: host%d jobSendRequestHandler could not create
//             requestX\n", host_id);
//     return;
//   }
//   strncpy(requestX->filename, job_from_queue->packet->payload,
//           PACKET_PAYLOAD_MAX);
...
//                             struct Job_queue *host_q, char *hostDirectory,
//                             struct Request **reqList) {
//   struct Packet *packet = job_from_queue->packet;
//   char *ticket, *filename, *fullPath;
...
//       ticket = (char *)malloc(sizeof(char) * TICKETLEN);
//       filename =
//           (char *)malloc(sizeof(char) * PACKET_PAYLOAD_MAX - TICKETLEN - 1);
//       parsePacket(packet->payload, ticket, filename);
...
//             "%s:Host%d does not have a valid directory set", ticket,
//             host_id);
//         goto SEND_RESPONSE;
//       }

//       // Build full file path from directory and filename
//       fullPath = (char *)malloc(sizeof(char) * 2 * MAX_FILENAME_LENGTH);
...
//         responseMsgLen =
//             snprintf(responseMsg, MAX_RESPONSE_LEN,
//                      "%s:file could not be created on host%d", ticket,
//                      host_id);
//         goto CLEANUP;
//       }
//       responseMsgLen =
...
//       struct Packet *uploadPkt = createPacket(packet->src, host_id,
//       PKT_UPLOAD); strncpy(uploadPkt->payload, packet->payload,
//       packet->length); uploadPkt->length = packet->length; struct Job
//       *uploadJob = createJob(JOB_WAIT_FOR_RESPONSE, uploadPkt);
...
//         strncpy(packet->payload, responseMsg, responseMsgLen);
//         packet->payload[responseMsgLen] = '\0';
//         packet->length = responseMsgLen;
//       } else {
//         // Handle error
//       }
//       // Create and enqueue job to send response packet
//       struct Job *responseJob = createJob(JOB_SEND_PKT, packet);
...
//       if (!fileExists(fullPath)) {
//         // Generate response message for non-existent file
//         responseMsgLen =
//             snprintf(responseMsg, MAX_RESPONSE_LEN,
...
//       // Generate response message for ready state
//       responseMsgLen =
//           snprintf(responseMsg, MAX_RESPONSE_LEN, "%s:Ready", ticket);
...
//       // Create and enqueue job to send response packet
//       struct Job *downloadResponseJob = createJob(JOB_SEND_PKT, packet);
//       job_enqueue(host_id, host_q, downloadResponseJob);

//       // If file exists, invoke jobUploadSendHandler
//       if (fileExists(fullPath)) {
//         jobUploadSendHandler(host_id, packet->dst, ticket, hostDirectory,
//                              filename, host_q);
//       }
...
//   char *ticket = (char *)malloc(sizeof(char) * TICKETLEN);
//   char *msg = (char *)malloc(sizeof(char) * PACKET_PAYLOAD_MAX - TICKETLEN -
//   1); parsePacket(pkt->payload, ticket, msg);
...
//       fflush(r->reqFp);  // Flush the file buffer to make sure data is
//       written
//                          // to disk
...
//   char *ticket = (char *)malloc(sizeof(char) * TICKETLEN);
//   char *msg = (char *)malloc(sizeof(char) * PACKET_PAYLOAD_MAX - TICKETLEN -
//   1); parsePacket(job_from_queue->packet->payload, ticket, msg);
...
//           break;
//         case DOWNLOAD_REQ:
//           snprintf(responseMsg, MAX_MSG_LENGTH, "Download request timed
//           out!"); sendMsgToManager(manFd, responseMsg); break;
//       }
...
//               jobUploadSendHandler(host_id, job_from_queue->packet->dst,
//               ticket,
...
//           // Create a job containing that packet
//           struct Job *requestJob = createJob(JOB_SEND_REQUEST,
//           requestPacket);
//           // And enqueue the job
...
//           int dst;
//           if (sscanf(man_msg, "%d %s", &dst, fname) != 2) {
//             colorPrint(BOLD_RED, "Invalid download request format\n");
//             write(man_port->send_fd, "", sizeof(""));
//             break;
//           }
...
//           snprintf(fullPath, sizeof(fullPath), "%s/%s", hostDirectory,
//           fname);
...
//           // User input is valid: enqueue download request to verify
//           destination struct Packet *dpacket = createPacket(host_id, dst,
//           PKT_DOWNLOAD_REQ); dpacket->length = strnlen(fname,
//           MAX_FILENAME_LENGTH); memcpy(dpacket->payload, fname,
//           dpacket->length);
//           // Create a job containing that packet
